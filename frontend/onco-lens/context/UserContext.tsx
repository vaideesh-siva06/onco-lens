...
export const UserProvider: React.FC<UserProviderProps> = ({ children }) => {
    ...
    const getUserInfo = async (id: string) => {
        try {
            const res = await axios.get(`http://localhost:8000/api/user/${id}`, { withCredentials: true });
            setUser(res.data);
        } catch (error) {
            console.error('Error fetching user info:', error);
            setUser(null);
    ...
    const updateUser = async (id: string, data: { name: string; email: string; password?: string }) => {
        try {
            const response = await axios.put(`http://localhost:8000/api/user/${id}`, data, { withCredentials: true });
